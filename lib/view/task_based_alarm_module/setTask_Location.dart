...
import 'Add_taskbased_alarm.dart';
import '../location_based_alarm_module/create_locationAlarm.dart';

class SettaskLocation extends StatefulWidget {
  const SettaskLocation({super.key});

  @override
  State<SettaskLocation> createState() => _SettaskLocationState();
}

class _SettaskLocationState extends State<SettaskLocation> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: CustomAppbar(title: "Select Location for Task", actions: [
        InkWell(
            onTap: () {_toggleMapType();},
            child: Icon(Icons.map,color: kPrimaryColor,))
      ],leading: IconButton(onPressed: (){Navigator.pop(context);}, icon: Icon(Icons.arrow_back_rounded,color: kPrimaryColor,)),),
      body: Stack(
        children: [
          Positioned.fill(
            child: GoogleMap(
        ...
        onPressed: () {
          ...
          if (_lastSelectedPosition != null) {
            ...
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => AddTaskbasedAlarm(
                  latitude: _lastSelectedPosition!.latitude.toDouble(),
                  longitude: _lastSelectedPosition!.longitude.toDouble(),
